//Made By ZeroFollowers
 ...
 setUpCanvas(width, height, center[true/false]) 
...
TRANSPARENCY ON SHAPE
ctx.globalAlpha = transparency; before you make your shape
ctx.globalAlpha = 1; after you make your shape
...
function setUpCanvas(width, height, center){if(center==true){var ee="display:block; margin: auto;";}else{var ee="display:block;"}var cd = document.createElement('canvas');cd.id = "myCanvas";cd.width = width;cd.height = height;var body = document.getElementsByTagName("div")[0];
body.appendChild(cd);cursorLayer = document.getElementById("CursorLayer");canvas = document.getElementById("myCanvas");ctx = canvas.getContext("2d");canvas.style = ee;}
...
function drawTransparentSquare(x, y, width, height, hex, trans){  ctx.beginPath(); ctx.globalAlpha=trans; ctx.rect(x, y, width, height); ctx.fillStyle = ("#"+ hex); ctx.fill(); ctx.globalAlpha =1; ctx.closePath(); }